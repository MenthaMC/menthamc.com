...
export class GitHubDataService {
  ...
  async getUserInfo(username: string): Promise<GitHubUser | null> {
    ...
    try {
      ...
      logger.error(`获取用户信息失败: ${username}`, error);
      return null;
    }
  }
  ...
  async getUserRepositories(username: string): Promise<GitHubRepository[]> {
    ...
    try {
      ...
      logger.error(`获取用户仓库失败: ${username}`, error);
      return [];
    }
  }
  ...
  async getRepository(owner: string, repo: string): Promise<GitHubRepository | null> {
    ...
    try {
      ...
      logger.error(`获取仓库信息失败: ${owner}/${repo}`, error);
      return null;
    }
  }
  ...
  clearCache(): void {
    ...
    // 清除本地缓存
    this.cache.clear();
    logger.debug(`GitHub数据服务缓存已清除: ${clearedCount} 项全局缓存 + ${this.cache.size} 项本地缓存`);
  }
